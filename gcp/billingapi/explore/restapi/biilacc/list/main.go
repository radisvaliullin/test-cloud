...
func main() {
	...
	resp, err := client.BillingAccounts.List().Context(ctx).Do()
	if err != nil {
		log.Printf("billing account list err - %v\n", err)
	}
	log.Printf("billing account list - %+v", resp)
	for _, l := range resp.BillingAccounts {
		log.Printf("billing account list, bill acc - %+v", l)
	}
}
